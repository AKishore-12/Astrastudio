...
class MyAgent:
    def __init__(self, id=None, role=None, backstory=None, goal=None, temperature=None, allow_delegation=False, verbose=False, cache= None, llm_provider_model=None, max_iter=None, created_at=None, tools=None, mcp_ids=None, knowledge_source_ids=None):
    ...
    def get_crewai_agent(self) -> Agent:
        ...
        # MCP Integration
        mcp_servers = []
        if 'mcps' in ss and self.mcp_ids:
            for mcp_id in self.mcp_ids:
                mcp = next((m for m in ss.mcps if m["id"] == mcp_id and m["enabled"]), None)
                if mcp:
    ...
    def draw(self, key=None):
        ...
        if self.edit:
            with st.expander(f"Agent: {self.role}", expanded=True):
                with st.form(key=form_key):
                    ...
                    if 'knowledge_sources' in ss and len(ss.knowledge_sources) > 0:
                        ...
                        # ---- MCP selection ----
                    if 'mcps' in ss and len(ss.mcps) > 0:
                        mcp_options = [m["id"] for m in ss.mcps]
                        mcp_labels = {m["id"]: m["name"] for m in ss.mcps}

                        mcp_key = f"mcp_sources_{self.id}_{key}" if key else f"mcp_sources_{self.id}"

                        selected_mcps = st.multiselect(
                            "MCP Servers",
                            options=mcp_options,
                            default=self.mcp_ids,
                            format_func=lambda x: mcp_labels.get(x, "Unknown"),
                            key=mcp_key,
                        )

                        self.mcp_ids = selected_mcps              
            ...
            with st.expander(expander_title, expanded=False):
                ...
                # Display MCP servers
                if self.mcp_ids and 'mcps' in ss:
                    selected_mcps = [m for m in ss.mcps if m["id"] in self.mcp_ids]
                    if selected_mcps:
