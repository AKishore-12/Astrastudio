...
def save_mcp(mcp: dict):
    ...
    save_entity("mcp", mcp["id"], data)


def load_mcps():
    rows = load_entities("mcp")
    mcps = []
    for row in rows:
        mcp_id, data = row
        mcps.append({
...
def load_agents():
    ...
    for row in rows:
        ...
        agent = MyAgent(id=row[0], knowledge_source_ids=knowledge_source_ids, mcp_ids=mcp_ids,**data)
